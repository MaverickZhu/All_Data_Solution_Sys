...
-- data_sources: 数据源配置表 (已优化架构)
CREATE TABLE data_sources (
    id SERIAL PRIMARY KEY,
    name VARCHAR(255) NOT NULL,
    project_id INTEGER NOT NULL REFERENCES projects(id),
    -- 新架构：双字段设计支持多模态数据
    file_type VARCHAR(50), -- 原始文件类型: 'docx', 'pdf', 'jpg', 'csv', etc.
    analysis_category analysis_category_enum NOT NULL DEFAULT 'UNSTRUCTURED', -- 分析类别
    file_path VARCHAR(1024) NOT NULL,
    file_size BIGINT,
    profile_status VARCHAR(50) DEFAULT 'pending', -- 'pending', 'in_progress', 'completed', 'failed'
    profiling_result JSONB,
    task_id VARCHAR(255),
    image_hash VARCHAR(255), -- 图像哈希值（用于相似图像检索）
    user_id INTEGER NOT NULL REFERENCES users(id),
...
-- 分析类别枚举类型
CREATE TYPE analysis_category_enum AS ENUM (
    'TEXTUAL',      -- 文本类数据 (txt, docx, pdf, md, etc.)
    'IMAGE',        -- 图像类数据 (jpg, png, gif, etc.)
    'TABULAR',      -- 表格类数据 (csv, xlsx, etc.)
    'VIDEO',        -- 视频类数据 (mp4, avi, etc.)
    'AUDIO',        -- 音频类数据 (mp3, wav, etc.)
    'GEOSPATIAL',   -- 地理空间数据 (geojson, shp, etc.)
    'GRAPH',        -- 图数据 (graphml, gexf, etc.)
    'UNSTRUCTURED'  -- 未结构化数据 (默认类别)
);
```
