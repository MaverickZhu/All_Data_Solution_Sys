...
### 🎬 视频深度分析关键问题 (2025-07-18调试发现)

- [ ] **任务卡顿根本原因分析**:
  - [⚠️] 任务在处理视频的特定帧(如第15帧)时卡住，前14帧正常处理
  - [⚠️] Worker重启后会自动重新执行队列中的任务，说明原任务并未真正失败只是卡住
  - [⚠️] 已排除：GPU内存不足(24GB充足)、网络问题、批次大小问题、Ollama服务问题
  - [ ] 需要添加帧级别详细日志，精确定位卡顿的具体原因
  - [ ] 实现任务超时和自动重试机制，防止任务无限期卡住
  - [ ] 添加LLM调用超时控制，避免特定内容导致的分析死循环

- [ ] **RabbitMQ队列管理优化**:
  - [ ] 实现卡住任务的自动检测和清理机制
  - [ ] 添加队列监控和管理API，便于手动清理问题任务
  - [ ] 优化任务重试策略，避免重复执行相同的失败任务
